...
Based on the results, compared with the baseline model, the trained LoRA model shows significant improvements in SQL correctness, execution success, and semantic similarity, indicating that, through fine-tuning, the general-purpose language model can adapt to the text-to-SQL task effectively while remaining computationally efficient.  The evaluation pipeline runs the predicted SQL queries against SQLite databases, and during the process, checks both the query results and text similarity. Therefore, the trained model performs much better in SQL validity, execution accuracy, and query similarity. These results demonstrate the effectiveness of LoRA-based adaptation for SQL generation tasks in low-resource computational settings.

## Introduction

Text-to-SQL generation aims to translate natural language questions into executable database queries. Although large language models tend to show high potential for zero-shot and few-shot inference, they lack precision when generating SQL. Since text-to-SQL requires an exact match, meaning even a single misplaced identifier or incorrect operator can cause errors. Moreover, SQL is always grounded in a specific schema: the model must understand which columns exist, how they relate to the question, and how to assemble a logically consistent query.
...
We evaluate the performance of the baseline Gemma model without LoRA fine-tuning, which serves as a reference for comparison with the Trained LoRA Model in the following section.
...
We evaluate the performance of the trained LoRA model (rank 32, alpha 16) to assess how task-specific fine-tuning improves text-to-SQL generation compared with the baseline Gemma model.
...
As we can see from Fig. 8, this plot shows the SQL prediction error breakdown for the trained LoRA Model. About 47.20% of the predictions have incorrect results; they have valid SQL but incorrect outputs. About 52.17% of the predictions have the correct output, matching the gold result. Only about 0.63% of the predictions fall into the SQL Error section. Compared with the baseline model, which produces only SQL errors, the LoRA-fine-tuned model shows a substantial improvement in generating executable and correct SQL queries.
...
As we can see from Fig. 9, this plot shows text-based evaluation metrics for the trained LoRA Model. A 95.230 BLEU score means there is a high similarity to the correct SQL wording. Also, a 0.971 ROUGE-L score means the structure of generated queries matches about 97.1% of the gold queries. Moreover, about 76.3% of generated queries exactly match the gold SQL. Lastly, the Token F1 score of 0.986 indicates that about 98.6% of the individual SQL tokens are correct, even though the full query is incorrect. Compared with the baseline model, these results demonstrate a substantial improvement in both SQL correctness and overall text similarity after LoRA fine-tuning.
...
As we can see from Fig. 9, this plot shows the distributions of Jaccard and normalized Levenshtein similarity between gold SQL queries and those generated by the trained LoRA Model. The similarity distributions are tightly concentrated around 1.0, indicating that, after fine-tuning, the SQL queries generated closely match the gold queries in both tokens and structure. Compared with the baseline model, the trained LoRA model shows a much stronger concentration of similarity scores.
...
As we can see from Fig. 7, this plot shows the relationship between similarity metrics and SQL execution correctness for the trained LoRA Model. We can see from the points indicating incorrect executions that correct SQL queries start to appear, clustered around high Jaccard and Levenshtein values. This means these queries have strong token overlap and a similar structure to the gold SQL. Also, the number of incorrect SQL queries decreased significantly. Compared with the baseline model, the trained LoRA model shows a clear relationship between similarity scores and SQL execution correctness.
...
As we can see from the above, for the same prompt, "What’s the title of the episode that Rob Heyland wrote?", the baseline model produces SQL that lacks matches. Such as incorrectly querying a non-existent table named Total instead of the data, which leads to an execution failure and fails to correctly align with the database schema. Where the trained LoRA Model is able to output an exact SQL query that matches the gold SQL query. Moreover, for Similarity Metrics, the baseline model shows partial token and structural overlap, while the trained LoRA model has perfect similarity scores. Additionally, in terms of execution, the baseline query fails to run, whereas the LoRA-generated query executes successfully and returns the correct result.

Overall compare with the baseline model, the trained LoRA model shows a strong improvements in SQL correctness, execution success, and semantic similarity.

## Discussion

### Limitations

**Restricted SQL complexity:** Training on WikiSQL limits the model to simple, single-table queries. It does not learn joins, grouping, nested logic, or multi-table reasoning needed for many real production systems.

**Shallow Schema Understanding:** he model only sees column headers without richer schema context (types, constraints, relationships), reducing its ability to generalize to new databases or ambiguous column naming.

**LoRA SFT Improves Syntax More Than Reasoning:** Fine Tuning mainly teaches formatting and column/operator patterns. It does not significantly enhance Multi Step reasoning, leading to remaining execution and logical failures.

### Future Work
We aim to includes adapting more complicated SQL data structures to the training pipeline, coordinating supervised fine tuning with other training methods like Reinforcement Learning, and integrating trained models into real world databases and web applications.
